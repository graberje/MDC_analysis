...
## -------------------------------------------------------------------------------------------------------------------------------------------------------------
# Primary dataset
dat <- read.table(file = "data/MotorItems.tab", fill = TRUE, header = TRUE, sep = "\t")

# Height/weight data
height.weight <- read.table(file = "data/HeightWeight.tab", fill = TRUE, header = TRUE, sep = "\t")

#Handedness
dat.hand <- read.table(file = "data/MotorScores.tab", fill = TRUE, header = TRUE, sep = "\t")
dat.hand <- dat.hand %>% dplyr::select(login, handedness) %>% group_by(login) %>% slice(1L)
...
dat.wide <- left_join(dat.wide, dat.hand, by = "login")



# Create dominant / non-dominant strength variables 

dat.wide %<>% mutate(dom.grip.1 = case_when(handedness == 1 ~ MSGSRT1_grip.1,
                                            handedness == 2 ~ MSGSLT1_grip.1),
                     
                     dom.grip.2 = case_when(handedness == 1 ~ MSGSRT1_grip.2,
                                            handedness == 2 ~ MSGSLT1_grip.2))


# Create change variables
dat.wide %<>% mutate(change = dom.grip.2 - dom.grip.1)
...
# Remove zero values
dat.wide <- dat.wide %>% filter(dom.grip.1 >0 & dom.grip.2 >0)



## -------------------------------------------------------------------------------------------------------------------------------------------------------------

table1(~ age + sex + bmi + dom.grip.1 + change,
       caption = "Table 1. Demographics of participants in grip strength dataset",
       data = dat.wide)
...
# histogram
meangrip<-mean(dat.wide$dom.grip.1)
his.line<-data.frame(mean=c(mean(dat.wide$dom.grip.1),mean(dat.wide$dom.grip.1)),
                     count = c(0,16))

his.grip<-ggplot(data = dat.wide)+
  geom_histogram(aes(x = dom.grip.1), fill = "lightblue", color = "black")+
  geom_line(aes(x=mean, y=count), data=his.line, linetype=2, size=2, color="black")+
  
  #ggtitle(label = "Distribution of change scores")+
  xlab(label = "Grip Strength (lbs); Initial Test")+
  ylab(label = "Count")+
  annotate(geom = 'text', x = meangrip, y = Inf, hjust = -0.1, vjust = 3,
           label = paste("Mean: ",
                         round(meangrip, digits = 2),
                         " lbs",
                         sep = ""), color="black")+
...
# Relationship between baseline grip and change
ggplot(data = dat.wide, aes(x = dom.grip.1, y = change))+
  geom_point()+
  
  geom_hline(yintercept = 0)+
  
  ggtitle(label = "Grip strength change by dom.grip.1")+
...
dat.icc <- pivot_longer(dat.wide,
                        cols = c(dom.grip.1, dom.grip.2),
                        values_to = "grip")

# Calculate ICC
icc <- ICC::ICCbare(x = login, y = grip, data = dat.icc)

  #95% CI
  icc.ci<-ICC::ICCest(x = login, y = grip, data = dat.icc, alpha = 0.05, CI.type = c("THD"))
  
# Calculate SEM with ICC
sem <- sd(dat.icc$grip) * sqrt(1 - icc)

  #95% CI
  icc.ci$LowerCI
  sem.mdc.lwr <- sd(dat.icc$grip) * sqrt(1 - icc.ci$LowerCI)
  sem.mdc.upr <- sd(dat.icc$grip) * sqrt(1 - icc.ci$UpperCI)
...
mdc90 <- sem * 1.645  * sqrt(2)
  ...
  #95% CI
  mdc90.lwr <- sem.mdc.upr* 1.645  * sqrt(2)
  mdc90.upr <- sem.mdc.lwr* 1.645  * sqrt(2)
...
mdc.prec <- mdc90 * 2
  ...
  #95%CI
  mdc.prec.lwr <- mdc90.lwr *2
  mdc.prec.upr <- mdc90.upr *2
...
dat.1 <- dat.wide %>% filter(dom.grip.1 < median(dat.wide$dom.grip.1))
...
dat.2 <- dat.wide %>% filter(dom.grip.1 > median(dat.wide$dom.grip.1))
...
# Calculate overall coverage for decline
...
# Calculate coverage for decline with baseline values below median
dat.1 <- dat.wide %>% filter(dom.grip.1 < median(dat.wide$dom.grip.1))
...
# Calculate coverage for decline with baseline values above median
dat.2 <- dat.wide %>% filter(dom.grip.1 > median(dat.wide$dom.grip.1))
...
ggplot(data = dat.wide, aes(x = dom.grip.1, y = change))+
  ...
  annotate(geom = 'text', x = Inf, y = Inf, hjust = 1, vjust = 1,
           label = paste("Mean precision: ",
                         round(mdc.prec, digits = 1),
                         " lbs",
                         sep = ""))+
  
  xlab(label = "Grip Strength (lbs); Initial Test")+
  ylab(label = "Test-Retest Difference (lbs)")+
  
  
  coord_cartesian(xlim = c(min(dat.wide$dom.grip.1), max(dat.wide$dom.grip.1)),
...
# Create model
m.slr <- lm(change ~ dom.grip.1, dat = dat.wide)
summary(m.slr)
...
# Overall precision
slr.prec <- mean(dat.wide$slr.upr - dat.wide$slr.lwr, na.rm = TRUE)+0.05

slrmin<-min(dat.wide$dom.grip.1)
slrmax<-max(dat.wide$dom.grip.1)
slr.data<-data.frame(dom.grip.1=seq(from=slrmin, to=slrmax, by=0.1))
...
slr.pred1$grip.1 <- slr.data$dom.grip.1

#write.csv(slr.pred1, file="slr_grip_080123.csv")

# Calculate the mean and standard error
l.slr <- lm(slr.prec ~ 1, dat.wide)

# Calculate the confidence interval
confint(l.slr, level=0.95)
...
# m.gamlss <- quote(gamlss(change + 100 ~cs(dom.grip.1, df=p[1]),
#                          sigma.formula = ~cs(dom.grip.1, df=p[2]),
#                          nu.formula = ~cs(dom.grip.1, df = p[3]),
#                          data=na.omit(dat.wide),
#                          family=BCT, n.cyc=500))
# 
# 
# 
# op <- find.hyper(model=m.gamlss, par=c(1,1,1), lower=c(0, 0, 0), steps=c(0.5,0.5,0.5), k=5)
# 
# op$par
...
# Model fit
m.gamlss <- gamlss(change + 100 ~(dom.grip.1),
                   sigma.formula = ~(dom.grip.1),
                   nu.formula = ~(dom.grip.1),
                   data=na.omit(dat.wide),
                   family=BCT, n.cyc=500)
...
gamlss.centiles <- gamlss.centiles %>% mutate(dom.grip.1 = x,
                                              gamlss.pred = `50` - 100,  
                                              gamlss.upr = `95` - 100,
                                              gamlss.lwr = `5` - 100,
                                              gamlss.prec = gamlss.upr  - gamlss.lwr) %>%
                                      dplyr::select(dom.grip.1, gamlss.upr, gamlss.lwr, gamlss.prec, gamlss.pred)

dat.final <- left_join(dat.wide, gamlss.centiles, by = "dom.grip.1")

dat.final <- distinct(dat.final)

minx<-min(dat.wide$dom.grip.1)
maxx<-max(dat.wide$dom.grip.1)
...
gamlss.centiles2$grip.1<-gamlss.centiles2$x+100

#write.csv(gamlss.centiles2, file="gamlss_grip_080123.csv")
...
# Calculate the mean and standard error
l.gamlss <- lm(gamlss.prec ~ 1, dat.final)

# Calculate the confidence interval
confint(l.gamlss, level=0.95)
...
# Coverage for improvement - Values below median
dat.1 <- dat.final %>% filter(dom.grip.1 < median(dat.final$dom.grip.1))
...
# Coverage for improvement - Values above median
dat.2 <- dat.final %>% filter(dom.grip.1 > median(dat.final$dom.grip.1))
...
ggplot(data = dat.final, aes(x = dom.grip.1))+
  ...
  geom_point(aes(x = dom.grip.1, y = change))+
...
plot.combined.grip <- ggarrange(mdc.plot, slr.plot, gamlss.plot,ncol = 3, nrow=1, common.legend = TRUE, legend="bottom")
plot.combined.grip 
